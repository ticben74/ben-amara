...
export const ShareModal: React.FC<Props> = ({ item, isOpen, onClose }) => {
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=${encodeURIComponent(landingUrl)}&color=0d9488&bgcolor=ffffff&margin=10`;
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-[3rem] max-w-2xl w-full overflow-hidden shadow-3xl flex flex-col animate-scale-up">
        <div className="p-8 border-b border-slate-800 flex items-center justify-between">
          ...
          <div className="text-right">
            <h4 className="text-2xl font-black text-white">مركز <span className="text-teal-500">الترويج الرقمي</span></h4>
        ...
        <div className="p-10 space-y-10">
          <div className="flex flex-col md:flex-row gap-10 items-center">
            <div className="bg-white p-6 rounded-[2.5rem] shadow-2xl shrink-0 group hover:rotate-2 transition-transform duration-500 border border-teal-100">
            ...
            <div className="space-y-6 flex-1 text-right">
              <div>
                ...
                <div 
                  ...
                  className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 font-mono text-[10px] text-teal-400 break-all select-all shadow-inner relative group cursor-pointer hover:bg-slate-900 transition-colors"
                >
                  {landingUrl}
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 justify-end">
                   <button onClick={copyToClipboard} className="px-4 py-2 bg-teal-600 hover:bg-teal-500 rounded-xl text-[10px] font-black text-white transition-all">نسخ الرابط</button>
                   <span className="px-3 py-2 bg-teal-500/10 border border-teal-500/20 rounded-xl text-[10px] font-black text-teal-400 uppercase">متوافق مع الهواتف</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            className="w-full bg-teal-600 hover:bg-teal-500 text-white font-black py-5 rounded-[2rem] shadow-2xl transition-all flex items-center justify-center gap-3 active:scale-95"
          >
